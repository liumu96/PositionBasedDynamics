...
template <typename HullType>
void KDTree<HullType>::construct(unsigned int node, AlignedBox3r const &box, unsigned int b,
                                 unsigned int n)
{
    // If only one element is left end recursion.
    // if (n == 1) return;
    if (n <= m_maxPrimitivesPerLeaf)
        return;
    ...
    auto c = static_cast<Real>(0.5) * (entity_position(m_lst[b + hal - 1])(max_dir) +
...
template <typename HullType>
void KDTree<HullType>::traverse_depth_first(TraversalPredicate pred, TraversalCallback cb,
...
template <typename HullType>
void KDTree<HullType>::traverse_depth_first(unsigned int node_index,
...
template <typename HullType>
void KDTree<HullType>::traverse_breadth_first(TraversalPredicate const &pred,
                                              TraversalCallback const &cb, unsigned int start_node, TraversalPriorityLess const &pless,
                                              TraversalQueue &pending) const
{
    cb(start_node, 0);
    if (pred(start_node, 0))
        pending.push({start_node, 0});
    traverse_breadth_first(pending, pred, cb, pless);
}

template <typename HullType>
void KDTree<HullType>::traverse_breadth_first_parallel(TraversalPredicate pred,
...
template <typename HullType>
void KDTree<HullType>::traverse_breadth_first(TraversalQueue &pending,
